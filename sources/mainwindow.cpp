...
{
	setWindowTitle("Technical drawing");
	main_event->setStyle("Fusion");
	main_event->setWindowIcon(QIcon(":/Logo.ico"));
	...
	QDockWidget* dock_renderer = new QDockWidget;
	dock_renderer->setWindowTitle("Renderer");
	dock_renderer->setWidget(renderer);
	dock_renderer->setFeatures(QDockWidget::DockWidgetMovable);
	addDockWidget(Qt::LeftDockWidgetArea, dock_renderer);
	...
	addDockWidget(Qt::RightDockWidgetArea, dock_diedric);
